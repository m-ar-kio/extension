...
import { MARK_CONTRACT, MAX_PST } from './constants'
...
export const getPSTAllocation = async () => {
  return calculateFeeRecipient(await getWalletList())
}

export const getWalletList = async () => {
  let tipTX = await findContractTip(MARK_CONTRACT)
  return JSON.parse(getTXState(tipTX)).walletList
}

const calculateFeeRecipient = (stakeholders) => {
  let weightedStakeholders = {}

  for (let i = 0; i < stakeholders.length; i++) {
    weightedStakeholders[stakeholders[i].addr] =
      stakeholders[i].balance / MAX_PST
  }
...
const getContract = async (contractID) => {
  ...
  const contractSrc = await contractSrcTX.get('data', {
    decode: true,
    string: true,
  })
...
const findNextTX = async (contract, state, currentTX) => {
  ...
  for (let i = 0; i < successors.length; i++) {
    ...
    if (validateNextTX(contract, state, TX)) return TX
  }
